...
import { buildSessionContext, compact, keyHint, type BranchSummaryResult, type CompactionEntry, type ExtensionAPI, type ExtensionContext, type ExtensionUIContext } from "@earendil-works/pi-coding-agent";
// compat: omp n'exporte pas generateBranchSummary.
import * as piAgentNs from "@earendil-works/pi-coding-agent";
const generateBranchSummary = (...args: any[]) => (piAgentNs as any).generateBranchSummary(...args);
...
function syncSharedSession(
	...
	// compat: omp injecte des custom_message (xdev-mount-notice) avant le
	// premier message ; rien n'est importé, save() n'écrit rien et la reprise
	// échoue ("No conversation found"). On repart d'une session propre.
	if (session.records.length === 0) {
		debug(`syncResult: path=clean-start (no convertible priors, ${priorMessages.length} skipped)`);
		return { sessionId: null };
	}
	session.save();
